...
- **Format hardening** — nested-object flattening, field sanitization and 10 MB
  guards for JSON and markdown; for XML, an **XXE-hardened `<add>`-only
  pre-check** so `<delete>`/`<commit>` cannot ride in on an indexing call.
