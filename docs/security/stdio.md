...
| `stdout` is reserved for JSON-RPC | `logback.xml` + `logback-spring.xml` declaring no appenders under the `stdio` profile (see [Logging Architecture in CLAUDE.md](../../CLAUDE.md#logging-architecture)) | Prevents log lines from being mis-parsed as MCP frames |
