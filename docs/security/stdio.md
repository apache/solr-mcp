...
| `stdout` is reserved for JSON-RPC | No appenders under the `stdio` profile in `logback-spring.xml` and `Main` silencing logback's own status output (see [Logging Architecture in CLAUDE.md](../../CLAUDE.md#logging-architecture)) | Prevents log lines from being mis-parsed as MCP frames |
