 ...
 * Apache Solr NamedList objects.
 *
 * <p>
 * This utility class simplifies the process of working with Solr's
 * {@code NamedList} response format by providing robust type conversion methods
 * that handle various data formats and edge cases commonly encountered when
 * processing Solr admin and query responses.
 *
 * <p>
 * <strong>Key Benefits:</strong>
 *
 * <ul>
 * <li><strong>Type Safety</strong>: Automatic conversion with proper error
 * handling
 * <li><strong>Null Safety</strong>: Graceful handling of missing or null values
 * <li><strong>Format Flexibility</strong>: Support for multiple input data
 * types
 * <li><strong>Error Resilience</strong>: Defensive programming against
 * malformed data
 * </ul>
 *
 * <p>
...
public class CollectionUtils {
	 ...
	 *
	 * <p>
	 * This method provides flexible extraction of Long values from Solr NamedList
	 * responses, handling various input formats that may be returned by different
	 * Solr endpoints. It performs safe type conversion with appropriate error
	 ...
	 *
	 * <ul>
	 * <li><strong>Number instances</strong>: Integer, Long, Double, Float,
	 * BigInteger, BigDecimal
	 * <li><strong>String representations</strong>: Numeric strings that can be
	 * parsed as Long
	 ...
	 *
	 * <p>
	 * Returns {@code null} for missing keys, null values, or unparseable strings
	 * rather than throwing exceptions, enabling graceful degradation in metrics
	 ...
	 *
	 * @param response
	 *            the NamedList containing the data to extract from
	 * @param key
	...
	/**
	 * Extracts a Float value from a NamedList using the specified key with
	 * automatic type conversion.
	 *
	 * <p>
	 * This method provides convenient extraction of Float values from Solr
	 * NamedList responses, commonly used for extracting percentage values, ratios,
	 * and performance metrics. It assumes that missing values should be treated as
	 * zero, which is appropriate for most metric scenarios.
	 *
	 * <p>
	 * <strong>Type Conversion:</strong>
	 *
	 * <p>
	 * Automatically converts any Number instance to Float using the
	 * {@link Number#floatValue()} method, ensuring compatibility with various
	...
	/**
	 * Extracts an Integer value from a NamedList using the specified key with
	 * robust type conversion.
	 *
	 * <p>
	 * This method provides flexible extraction of Integer values from Solr
	 * NamedList responses, handling various input formats that may be returned by
	 * different Solr endpoints. It performs safe type conversion with appropriate
	 ...
	 *
	 * <p>
	 * <strong>Type Conversion Strategy:</strong>
	 *
	 * <p>
	 * For Number instances, uses {@link Number#intValue()} which truncates decimal
	 * values. For string values, attempts parsing with
	 * {@link Integer#parseInt(String)} and returns null if parsing fails rather
	 ...
	 *
	 * <p>
	 * <strong>Range Considerations:</strong>
	 *
	 * <p>
	 * For large values that may exceed Integer range, consider using
	 * {@link #getLong(NamedList, String)} instead to avoid truncation or overflow
	 * issues.
	 *
	 * @param response
	 *            the NamedList containing the data to extract from
