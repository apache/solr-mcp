...
/**
 * Utility class for sanitizing field names to ensure compatibility with Solr's
 * field naming requirements.
 *
 * <p>
 * This class provides shared regex patterns and sanitization logic that can be
 * used across all document creators to ensure consistent field name handling.
 *
 * <p>
 * Solr has specific requirements for field names that must be met to ensure
 * proper indexing and searching functionality. This utility transforms
 * arbitrary field names into Solr-compliant identifiers.
 */
public final class FieldNameSanitizer {

	/**
	 * Pattern to match invalid characters in field names. Matches any character
	 * that is not alphanumeric or underscore.
	 */
	private static final Pattern INVALID_CHARACTERS_PATTERN = Pattern.compile("[\\W]");

	/**
	 * Pattern to match leading and trailing underscores. Uses explicit grouping to
	 * make operator precedence clear.
	 */
	private static final Pattern LEADING_TRAILING_UNDERSCORES_PATTERN = Pattern.compile("(^_+)|(_+$)");

	/**
	 * Pattern to match multiple consecutive underscores. Matches two or more
	 * consecutive underscores to collapse them into one.
	 */
	...
	/**
	 * Sanitizes field names to ensure they are compatible with Solr's field naming
	 * requirements.
	 *
	 * <p>
	 ...
	 * <li><strong>Character Replacement</strong>: Non-alphanumeric characters
	 * replaced with underscores
	 * <li><strong>Edge Trimming</strong>: Leading and trailing underscores removed
	 * <li><strong>Duplicate Compression</strong>: Multiple consecutive underscores
	 * collapsed to single
	 * <li><strong>Numeric Prefix</strong>: Field names starting with numbers get
	 * "field_" prefix
	 * </ul>
	 *
	 * <p>
