...
removed in Solr 10; `getCacheMetrics()` and `getHandlerMetrics()` now use the `/admin/metrics`
API (available since Solr 7.1+) for cache and handler metrics, ensuring compatibility with
both Solr 9 and 10.
...
- **SolrJ 10.x dependency:** Not yet on Maven Central; tests use SolrJ 9.x against a Solr 10 server. Update `solr-solrj` and Jetty BOM when 10.x is released.
