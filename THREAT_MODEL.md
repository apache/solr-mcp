...
6. **XML indexing is XXE-hardened.** `SolrUpdateXml` reads the payload up to its
   root element with a StAX reader with DTD support off and external entities off,
   rejects any DOCTYPE outright, and Solr's own XML loader behind it applies the
   same hardening.
...
- **"XXE in XML indexing."** The StAX pre-check runs with DTD support and
  external entities off and rejects any DOCTYPE, and the payload then goes to
  Solr's own hardened XML loader. `KNOWN-NON-FINDING`. *(documented —
  `SolrUpdateXml`.)*
